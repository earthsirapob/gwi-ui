...
import React__default, { forwardRef, useContext, Children, isValidElement, cloneElement, createContext, useState, useMemo } from 'react';
import * as ReactDOM from 'react-dom';
import ReactDOM__default from 'react-dom';
import { createTheme as createTheme$3, ThemeProvider as ThemeProvider$1 } from '@mui/material/styles';
...
function processStyleArg(callableStyle, _ref) {
  let {
    ...
    props = (0, _objectWithoutPropertiesLoose2.default)(_ref, _excluded$z);
...
const _excluded$o = ["disableSpacing", "className"];
...
const MyTextField = ({ label, value, onChange, ...props }) => {
    return (jsxRuntimeExports.jsx(TextField, { label: label, value: value, onChange: onChange, ...props }));
};
...
const ThemeContext = createContext({
    toggleTheme: () => { },
    setTheme: () => { },
    theme: createTheme$3(lightThemeJson), // Default theme
    themeName: 'light',
});
const useTheme = () => useContext(ThemeContext);
const ThemeProvider = ({ children }) => {
    const [themeName, setThemeName] = useState('light');
    const theme = useMemo(() => createTheme$3(themes[themeName]), [themeName]);
    const toggleTheme = () => {
